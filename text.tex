...
\begin{definition}
Řídící stav sekvencializovaného plochého paralelního NTS s k vlákny je k-tice $q = \tuple{q_0, \ldots, q_{k-1}}$, kde $q_i$ je řídící stav BasicNts, který je vykonáván ve vlákně s $\texttt{tid} = i$.
\end{definition}
V souladu s tím, co bylo řečeno v úvodu \todo{říci to v úvodu}, řídící stavy sekvencializovaného NTS nenesou informaci o valuaci datových proměnných. \todo{Pokud ano, tak pouze pro optimalizaci - v budoucnu.} S každým řídícím stavem $q$ sekvencializovaného NTS je spojena množina přechodů $\texttt{Outgoing}(q)$, vedoucích z tohoto stavu.
\begin{equation}
\texttt{Outgoing}(q) = \texttt{Outgoing}(q_0) \cup \ldots \cup \texttt{Outgoing}(q_{k-1})
\end{equation}

V rámci zachování jednoduchosti zde používáme o jednodušší definici stavu, než v \cite{NTSref}, zejména vypouštíme definici valuace proměnných.
\begin{definition}
Stav $s$ sekvencializovaného přechodového systému je dvojice $s = \tuple{q, v}$, kde $v$ je řídící stav daného systému a $v$ značí valuaci (globálních i všech lokálních) proměnných.
\end{definition}

\todo{Je třeba rozlišit mezi přechody v paralelním, naivně sekvencializovaném a redukovaném systému.}
...
\todo{Jak může úloha aktivovat jinou a co z toho plyne. Je ale třeba mít popsané základy POR.}
